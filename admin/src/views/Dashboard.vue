<template>
  <div class="dashboard">
    <div class="dashboard-header">
      ...
      <p>Chào mừng trở lại, {{ authStore.user?.fullName || authStore.user?.email }}!</p>
      <p>Vai trò: {{ authStore.user?.role }} | Trạng thái: {{ authStore.user?.status }}</p>
    </div>
            ...
            <template #renderItem="{ item }">
              <a-list-item v-if="item && item.user">
                <a-list-item-meta>
                  ...
                  <template #description>
                    Submitted {{ formatDate(item.submittedAt || new Date().toISOString()) }}
                  </template>
                </a-list-item-meta>
                <template #actions>
...
const loadDashboardData = async () => {
  ...
  try {
    ...
    // TODO: Add API calls for recent users and KYC when available
